export class RenderEngine {
    ...
    drawMaze() {
        ...
        for (let y = 0; y < this.game.mazeRowCount; y++) {
            for (let x = 0; x < this.game.mazeColCount; x++) {
                ...
                this.ctx.fillRect(x * this.game.cellSize, y * this.game.cellSize + this.game.topBorderSize, this.game.cellSize, this.game.cellSize);
            }
        }
    }
    drawTank() {
        this.ctx.save();
        this.ctx.translate(this.game.tank.pos.x * this.game.cellSize + this.game.cellSize / 2, this.game.tank.pos.y * this.game.cellSize +
        ...
        for (let y = 0; y < 16; y++) {
            ...
            for (let x = 0; x < 16; x++) {
                if (row[x] !== "0") {
                    ...
                    this.ctx.fillRect(x * cellSize - center, y * cellSize - center, cellSize, cellSize);
                }
            }
        }
        this.ctx.restore();
    ...
    drawTargets() {
        this.game.targets.forEach((target) => {
            if (!target.hit || target.flashTimer > 0) {
                const targetRadius = (this.game.cellSize / 2) * this.CONFIG.TARGET_RADIUS_SCALE;
                const x = target.pos.x * this.game.cellSize + this.game.cellSize / 2;
                const y = target.pos.y * this.game.cellSize +
                    this.game.cellSize / 2 +
                    this.game.topBorderSize;
                this.ctx.fillStyle = this.CONFIG.TARGET_OUTLINE_COLOR;
                this.ctx.beginPath();
                this.ctx.arc(x, y, targetRadius + this.CONFIG.TARGET_OUTLINE_WIDTH, 0, Math.PI * 2);
                this.ctx.fill();
                this.ctx.fillStyle = target.color;
                this.ctx.beginPath();
                this.ctx.arc(x, y, targetRadius, 0, Math.PI * 2);
                this.ctx.fill();
                if (this.game.numberTimer > 0 ||
                    target.flashTimer > 0 ||
                    (this.game.showNextTimer > 0 &&
                        target.num === this.game.currentTarget)) {
                    const alpha = target.flashTimer > 0
                        ? Math.max(0, target.flashTimer / this.CONFIG.FLASH_DURATION)
                        : 1;
                    this.ctx.fillStyle = `rgba(255, 255, 255, ${alpha})`;
                    const text = target.num.toString();
                    this.ctx.font = `${this.CONFIG.TARGET_FONT_SIZE / Math.sqrt(text.length)}px Arial`;
    ...
    drawChaosMonster() {
        if (this.game.chaosMonster) {
            this.ctx.save();
            this.ctx.translate(this.game.chaosMonster.pos.x * this.game.cellSize +
                this.game.cellSize / 2, this.game.chaosMonster.pos.y * this.game.cellSize +
                this.game.cellSize / 2 +
                this.game.topBorderSize);
    ...
    drawPowerUp() {
        this.game.powerUps.forEach((p) => {
            const radius = (this.game.cellSize / 2) * this.CONFIG.POWER_UP_RADIUS_SCALE;
            this.ctx.fillStyle = `rgba(255, 255, 0, ${p.opacity})`;
            this.ctx.beginPath();
            this.ctx.arc(p.pos.x * this.game.cellSize + this.game.cellSize / 2, p.pos.y * this.game.cellSize +
                this.game.cellSize / 2 +
                this.game.topBorderSize, radius, 0, Math.PI * 2);
            this.ctx.fill();
        });
    }
    drawBullets() {
        this.game.bullets.forEach((bullet) => {
            ...
            this.ctx.beginPath();
            this.ctx.arc(bullet.pos.x * this.game.cellSize + this.game.cellSize / 2, bullet.pos.y * this.game.cellSize +
                this.game.cellSize / 2 +
                this.game.topBorderSize, this.CONFIG.BULLET_SIZE, 0, Math.PI * 2);
            this.ctx.fill();
        });
    }
    drawMarker() {
        if (this.game.marker) {
            const x = this.game.marker.x * this.game.cellSize + this.game.cellSize / 2;
            const y = this.game.marker.y * this.game.cellSize +
                this.game.cellSize / 2 +
                this.game.topBorderSize;
    ...
    drawScoreboard() {
        ...
        this.ctx.fillStyle = "white";
        const bannerHeight = this.game.canvas.width * this.CONFIG.BANNER_HEIGHT_RATIO;
    ...
    drawMessages() {
        if (this.game.gameOver || this.game.levelCleared) {
            ...
            this.ctx.fillText(message, this.game.canvas.width / 2, this.game.canvas.height / 2);
        }
    }
}
