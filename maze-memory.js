...
export class MazeMemoryGame {
    constructor() {
        ...
        if (this.CONFIG.DEVICE_FLAG.isTouchSupported) {
            ...
            this.canvas.addEventListener("touchstart", this.handleCanvasTap.bind(this));
        }
        else {
    ...
    resetLevel(restartSameLevel = false) {
        if (this.gameOver && !restartSameLevel)
            return;
        const levelCycle = (this.level - 1) % this.CONFIG.LEVELS_PER_CYCLE;
        const difficulty = levelCycle + Math.floor((this.level - 1) / this.CONFIG.LEVELS_PER_CYCLE);
        this.updateCanvasSize();
        ...
        if (this.level < this.CONFIG.POWER_UP_START_LEVEL) {
            ...
            const powerUpCount = Math.min(Math.floor((this.level - this.CONFIG.POWER_UP_START_LEVEL) /
                this.CONFIG.LEVELS_PER_CYCLE), this.CONFIG.MAX_POWER_UP_COUNT);
    ...
    updateCanvasSize() {
        ...
        const majorCellCount = makeOdd(clamp(this.CONFIG.MIN_CELL_COUNT + Math.floor(this.level / 3), this.CONFIG.MIN_CELL_COUNT, this.CONFIG.MAX_CELL_COUNT));
        const cellSize = Math.max(Math.floor(gridSize.x / majorCellCount), Math.floor(gridSize.y / majorCellCount));
        const minorCellCount = makeOdd(clamp(Math.min(Math.floor(gridSize.x / cellSize), Math.floor(gridSize.y / cellSize)), this.CONFIG.MIN_CELL_COUNT, this.CONFIG.MAX_CELL_COUNT));
        ...
        this.cellSize = Math.min(Math.floor(gridSize.x / minorCellCount), Math.floor(gridSize.y / minorCellCount));
    ...
    getRandomOpenPosition() {
        ...
        do {
            pos = new Vector2D(Math.floor(Math.random() * this.mazeColCount), Math.floor(Math.random() * this.mazeRowCount));
            attempts++;
    ...
    isValidMove(pos) {
        if (this.tank.ignoreCollisions)
            return true; // Bypass check during wall traversal
        const { x, y } = pos.round();
        return (x >= 0 &&
            x < this.mazeColCount &&
            y >= 0 &&
    ...
    isIntersection(pos, fromDir) {
        ...
        directions.forEach((d) => {
            ...
            if (this.isValidMove(newPos) && d.dir !== fromDir)
                openPaths++;
        });
        return openPaths > 1;
    }
